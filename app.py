...
def fuse(r,m):
 if r is None:return m,'Microbiome',0,1
 if m is None:return r,'RNA',1,0
 rs=2.91 if r<20 else 50.; ms=34.06; wr=1/rs**2; wm=1/ms**2; return (wr*r+wm*m)/(wr+wm),('RNA' if wr>=wm else 'Microbiome'),wr/(wr+wm),wm/(wr+wm)
...
with st.sidebar:
 st.markdown('## 🔬 ForensicChrono'); st.caption('PMI Intelligence Console'); case=st.text_input('Case ID','FC-2026-001'); choice=st.radio('Biological evidence',['RNA only','Microbiome only','RNA + Microbiome'],index=2); st.markdown('---'); st.caption('Research prototype • Decision support only')
st.markdown('<div class="hero"><div class="brand">Forensic<span>Chrono</span></div><div class="tagline">Multimodal biological intelligence for postmortem interval estimation</div><div class="badge">RNA degradation • microbial succession • reliability-weighted fusion</div></div>',unsafe_allow_html=True)
a,b,c=st.columns(3)
for col,label,ready,small in [(a,'RNA model',rna_pkg,'Tissue-specific stacked ensemble'),(b,'Microbiome model',micro_pkg,'Microbial succession ensemble'),(c,'Explainability',SHAP_OK,'Case-specific XGBoost SHAP')]:
 with col:st.markdown(f'<div class="metric-card"><div class="metric-label">{label}</div><div class="metric-value">{"READY" if ready else "MISSING"}</div><div class="metric-small">{small}</div></div>',unsafe_allow_html=True)

r_hours=m_hours=None;r_tab=m_tab=None;r_fig=m_fig=None;tissue='Not applicable'
if choice in ['RNA only','RNA + Microbiome']:
 st.markdown('<div class="section"><div class="section-title">🧬 RNA evidence intake</div><div class="section-sub">Upload the expression data. PMI is never entered manually.</div></div>',unsafe_allow_html=True)
 tissue=st.selectbox('Tissue type',TISSUES,index=1); rf=st.file_uploader('RNA expression CSV / TSV',type=['csv','tsv','txt'],key='rf'); x1,x2,x3=st.columns(3); rin=x1.number_input('RIN',0.,10.,7.,.1); aut=x2.number_input('Autolysis score',0.,20.,0.,.1); age=x3.number_input('Age',0.,120.,50.,1.); x4,x5=st.columns(2); sex=x4.selectbox('Sex',['Female','Male','Unknown']); hardy=x5.number_input('Hardy death scale',0.,4.,0.,1.)
 if rf:
  try:
   sep='\t' if rf.name.lower().endswith(('.tsv','.txt')) else ','; raw=pd.read_csv(io.BytesIO(rf.getvalue()),sep=sep,low_memory=False); first=raw.columns[0]
   if raw.shape[1]==2: expr=pd.DataFrame([raw.iloc[:,1].astype(float).values],columns=raw.iloc[:,0].astype(str))
   elif raw.shape[0]==1: expr=raw.copy(); expr.columns=expr.columns.astype(str)
   else: expr=raw.set_index(first).T; expr.columns=expr.columns.astype(str)
   md=pd.DataFrame([{'rin':rin,'autolysis':aut,'age':age,'sex':1 if sex=='Male' else 0 if sex=='Female' else np.nan,'hardy':hardy if hardy>0 else np.nan}]); r_hours,rX,rnames,rxgb=predict_rna(rna_pkg,tissue,expr,md); st.success(f'RNA prediction: **{pmi(r_hours)}**'); r_tab,r_fig=shap_one(rxgb,rX,rnames)
  except Exception as e: st.error(f'RNA processing error: {e}')
if choice in ['Microbiome only','RNA + Microbiome']:
 st.markdown('<div class="section"><div class="section-title">🦠 Microbial succession evidence</div><div class="section-sub">Upload the OTU table using the same taxon ordering as the trained model.</div></div>',unsafe_allow_html=True)
 mf=st.file_uploader('Microbiome / OTU CSV / TSV',type=['csv','tsv','txt'],key='mf'); q1,q2=st.columns(2); indoor=q1.number_input('Indoor ADD',0.,10000.,0.,.1); outdoor=q2.number_input('Outdoor ADD',0.,10000.,0.,.1); q3,q4=st.columns(2); env=q3.selectbox('Environment',['unknown','indoor','outdoor','indoor_outdoor']); site=q4.text_input('Body site','unknown')
 if mf:
  try:
   sep='\t' if mf.name.lower().endswith(('.tsv','.txt')) else ','; raw=pd.read_csv(io.BytesIO(mf.getvalue()),sep=sep,index_col=0,low_memory=False); raw=raw.apply(pd.to_numeric,errors='coerce').fillna(0); otu=raw.T.iloc[[0]]; m_hours,mX,mnames,mxgb=predict_micro(micro_pkg,otu,indoor,outdoor,env,site); st.success(f'Microbiome prediction: **{pmi(m_hours)}**'); m_tab,m_fig=shap_one(mxgb,mX,mnames)
  except Exception as e: st.error(f'Microbiome processing error: {e}')

if r_hours is not None or m_hours is not None:
 final,primary,rw,mw=fuse(r_hours,m_hours); st.markdown('---'); st.markdown(f'<div class="result"><div class="result-cap">Estimated postmortem interval</div><div class="result-num">{pmi(final)}</div><div class="note">Primary evidence source: <b>{primary}</b></div></div>',unsafe_allow_html=True); z1,z2,z3=st.columns(3)
 z1.metric('RNA estimate',pmi(r_hours) if r_hours is not None else 'Not available'); z2.metric('Microbiome estimate',pmi(m_hours) if m_hours is not None else 'Not available'); z3.metric('Assessment', 'Fused' if r_hours is not None and m_hours is not None else 'Single modality')
 if r_tab is not None or m_tab is not None:
  tabs=st.tabs((["RNA"] if r_tab is not None else [])+(["Microbiome"] if m_tab is not None else [])); i=0
  if r_tab is not None:
   with tabs[i]: st.pyplot(r_fig,use_container_width=True); st.dataframe(r_tab,use_container_width=True,hide_index=True)
   i+=1
  if m_tab is not None:
   with tabs[i]: st.pyplot(m_fig,use_container_width=True); st.dataframe(m_tab,use_container_width=True,hide_index=True)
  
 data=pdf(case,tissue,'RNA + Microbiome' if r_hours is not None and m_hours is not None else ('RNA' if r_hours is not None else 'Microbiome'),r_hours,m_hours,final,primary,rw,mw,{'RNA':r_tab,'Microbiome':m_tab})
 if data: st.download_button('📄 Download Professional Forensic Report',data=data,file_name=f'ForensicChrono_{case}.pdf',mime='application/pdf',type='primary')
 st.markdown('<div class="warn"><b>Interpretation:</b> This system is a research decision-support tool and should not be used as a standalone determination of time of death. Results should be interpreted together with conventional forensic evidence.</div>',unsafe_allow_html=True)
else: st.markdown('<div class="note"><b>Ready for evidence intake.</b><br>Select the available modality, upload the biological data, and ForensicChrono will generate the PMI estimate automatically.</div>',unsafe_allow_html=True)
